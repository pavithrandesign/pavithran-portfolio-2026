...
- Below 1000px, the rail becomes an off-canvas panel (`position: fixed; transform: translateX(-100%)`, slides in via `.is-open`), the `.mobile-bar` hamburger bar appears, and `.cs-section[id] { scroll-margin-top: 64px; }` so anchor jumps clear the sticky mobile bar. (Case 2 and Case 3 originally shipped this at 900px, a small inherited inconsistency; reconciled to 1000px, matching Case 1 exactly, in the Sep 22 consistency audit below.)
...
- **Mini-header eyebrows**: numbers dropped from the text (`"01 · Overview"` → `"Overview"`), font-size bumped from the shared `--fs-micro` (0.75rem) to `1rem`, weight bumped to `700` (matching Case 1's `.eyebrow` weight exactly, confirmed in the Sep 22 consistency audit below), and moved to sit **directly above** `.cs-headline`, flush with no gap, instead of in a separate sticky `.cs-section-label` left column — i.e. `.cs-section-inner { grid-template-columns: 1fr; }` and `.cs-section-label { position: static; margin-bottom: 0; }`, both page-local overrides. This matches Case 1's slide pattern exactly (`<p class="eyebrow">The Problem</p>` sitting flush directly above `<h2 class="headline">`, zero gap, 700 weight), single column throughout, not the 0.8fr/2.2fr split described for the untouched template in Section 5.
...
## ⚠️ Consistency audit: Case 2/3 rail chrome reconciled to Case 1 exactly (Sep 22, 2026)

After the left-rail nav port above, a precise computed-style audit (Playwright, `getComputedStyle()`/`getBoundingClientRect()` measurements, not visual inspection) compared Case 2 and Case 3 against Case 1 and found four remaining small drifts, since fixed so all three case studies now match exactly:

- **Eyebrow-to-headline gap**: was `margin-bottom: var(--sp-5)` (24px) on `.cs-section-label` in Case 2/3, now `0` (flush), matching Case 1's `.eyebrow` sitting directly against `.headline` with no gap.
- **Eyebrow font-weight**: was inheriting the shared `.eyebrow` weight (600) in Case 2/3, now explicitly `700` in the page-local override, matching Case 1's `.eyebrow` weight.
- **Rail-agenda link radius**: was `var(--r-sm)` (16px, the shared token) in Case 2/3, now hardcoded `12px` in the page-local override, matching Case 1's local `.agenda a` radius (Case 1 doesn't link the shared stylesheet, so its `--r-sm` is a different, page-local 12px value; Case 2/3 needed a hardcoded `12px` to match it exactly rather than pointing at the shared token).
- **Mobile breakpoint**: was `900px` in Case 2/3, now `1000px`, matching Case 1 (see the breakpoint note above).

Everything else audited (rail width/padding, back-link color and spacing, `.rail-head h1` type, `.rail-contact` footer style, headline weight, body background, border tokens) was already consistent and needed no change. Pushed to git as commit `8563d2e`.
...
| `--fs-micro` | 0.75rem | Eyebrows, chips, tags, captions (Case 2/Case 3 section-label eyebrows are bumped to a literal 1rem and weight 700 instead, matching Case 1's `.eyebrow` exactly, see the Sep 22 notes above) |
...
- The full left-rail nav pattern (`.shell`/`.rail`/`.rail-agenda`/`.mobile-bar`/`.nav-toggle`/`.rail-backdrop` + the two nav IIFEs), see the Sep 22 notes above, now identical (breakpoint, eyebrow gap/weight, and agenda-link radius all reconciled by the consistency audit) across all three case studies.
